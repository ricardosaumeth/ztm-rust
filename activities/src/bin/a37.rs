...
impl TryFrom<&str> for Rgb {
  type Error = RgbError;
  fn try_from(hex_value: &str) -> Result<Self, Self::Error> {
    ...
    let (r, g, b) = (
      u8::from_str_radix(&hex_value[1..=2], 16)?,
      u8::from_str_radix(&hex_value[3..=4], 16)?,
      u8::from_str_radix(&hex_value[5..=6], 16)?,
    );
